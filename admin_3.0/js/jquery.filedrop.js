...
;(function($) {
  ...
  $.fn.filedrop = function(options) {
	...
	opts.droparea = this;
	opts.fallback = this.find('input[type="file"]').eq(0);
		
	if (opts.fallback){
		opts.fallback.change(function(e) {
		  opts.drop(e);
		...
		if (!opts.paramname)
			opts.paramname = opts.fallback.attr('name') ;
		if (!opts.url)
			opts.url = opts.fallback.data('url') ;
	}
    function drop(e) {
    ...
    function getBuilder(filename, filedata, mime, boundary) {
      ...
      builder += '; filename="' + filename.replace(/[^a-zA-Z0-9\.]+/g,'-') + '"';
      builder += crlf;
