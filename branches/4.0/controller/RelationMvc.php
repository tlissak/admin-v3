...
class RelationMvc{
    ...
    public function GetState($r,$data,$titleField){
    ...
    public function GetTabsCont(){
        ...
        foreach($this->parent->relations_instances as $r){
            ...
            $related_table = Loader::Get($r->name) ;

            $related_table->tmpView = $r->view_type ;

            $tabs_cont[] = '<div class="tab-pane" id="tab-relation-'.$r->name.'">' ;

            if ($this->parent->id) {
                $tabs_cont[] = $this->GetState($r, $this->parent->Form->data, $related_table->titleField);
            ...
            if ($r->data['type'] != 'ManyToMany' && $r->data['type'] != 'ManyToOneByKey' ) {
                $tabs_cont[] = $related_table->ListingMvc->GetPanel();
            }


            $tabs_cont[] = '</div>';

            $related_table->tmpView = '' ;
