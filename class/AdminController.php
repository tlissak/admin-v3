...
#region REM
	...
	*/
        ...
        if (! $tbl){
            p('No tbl param or tbl');
            die ;
        }
        if (  ! isset(self::$tableInstances[$tbl])) {
            p('Table not init in table instances');
            p($tbl);
            p(array_keys(self::$tableInstances)) ;
            die ;
        }
      ...
      // p(self::$tableInstances['images']);


        if ($contexttbl && ($this->table->name != $contexttbl )){
            $table = self::$tableInstances[$contexttbl] ;
            $table->initRelationsObject() ;//
            //$table->relations ;
...
*/
        ...
        if(get("v4_get_list_ajax")){
            ...
            $rows = $this->contextTable->getTableRows() ;
            $out = array('total'=> $this->contextTable->num_results,"status"=>200 ,'rows'=>$rows);
            //for debug disable this line
            //header('Content-type: application/json');
            ...
            $this->action	= 'add' ;
            $this->contextTable->v4_initList($ctrl->PAGE_SIZE)  ; // even if it calles by ajax page size limit should not requires a lot of resource
