...
*/
    ...
    public function GetPanels(){
        ...
        $this->parent->formPanel = array_merge( array('default'=>array('title'=>$this->parent->title,'icon'=>'compose','cont'=>'','pull'=>"pull-right-lg")) , $this->parent->formPanel );
        ...
        foreach($this->parent->formPanel as $key=>&$pnl){
            ...
            $out .= $this->parent->Mvc->RenderPanel($key,$pnl) ;
        }
    ...
    private function Control($a)
    {
        ...
        $opt = $a['opts'];
        $type = $a['type'];
        $d = $this->parent->Form->data; //? $this->data : array($fld=>post($fld) ) ;
...
// if (! array_key_exists($fld,$d)){fb('Trying to create field  "'. $fld . '" but the table ("'.$this->name.'")  does not have this field  ');}
        ...
        if ($type == 'checkbox') {
            ...
            $out .= '<label class="form-checkbox form-icon">
        ...
        if ($type == 'file') {
            ...
            if (V2_IMG && is_image('../' . $d[$fld])) {
                ...
                if (is_image(P_PHOTO . $d[$fld])) {
                    $h .= '<img src="' . U_PHOTO . $d[$fld] . '?width=200" style="max-height:100px; max-width:100px;"  />';
                } elseif (is_pdf(P_PHOTO . $d[$fld])) {
                    $h .= '<a href="' . U_PHOTO . $d[$fld] . '" target="_blank"><img src="img/pdf.jpg"  /></a>';
                } else {
                    $h .= $d[$fld];
                }
            } else {
