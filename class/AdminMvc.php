...
class AdminMvc extends AdminList{
    ...
    public function v4_getTableHtml(){
        ...
        $out =  '<table
               ...
               data-url="?tbl='.$tbl.'&v4_get_list_ajax=1"
	...
	public function v3_getTableHtml(){
	...
	function v3_getTableBody(){ // created for paginig w ajax
	...
	public function v3_getTableRowById($id){
	...
	function v3_getTableRow($dt = array()){
			...
			foreach ($this->view as $k=>$v){
				if ( ! array_key_exists ($k,$dt)){
					//if (DEV_MODE) Debug (' Check The row dosent contains key ' . $k);
					$tr .= '<td>'.$id .'!</td>' ;
				}else{
					if ($dt[$k] != null){
						$tr .= '<td>'. $dt[$k] .'</td>' ;
    ...
    function v4_getTableRow($dt = array()){
        ...
        //has checkbox
        //if ($this->viewtype == "SELECT-EDIT" && !$this->protected)
        //$dt['_selected'] ? 'checked="checked"'
        //id="_fld_'.$this->name.'_'.$id .'" value="'.$id .'"
        //class="relation-cb relation-cbx"
