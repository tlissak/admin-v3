	...
	function nextSlide(nav){    //funzione per il fading delle immagini
		...
		switch(fx){
			...
			case 'curtainSliceRight':
				cols = (opts.slicedCols == 0)  ? opts.cols :  opts.slicedCols;
				rows = 1;
					break;
			...
			case 'blindCurtainSliceBottom':
				rows = (opts.slicedRows == 0)  ? opts.rows :  opts.slicedRows;
				cols = 1;
					break;
			case 'stampede':
			...
			switch(fx){
				...
				case 'curtainTopRight':					arr = arr.reverse();				break;
				case 'curtainBottomRight':			arr = arr.reverse();				break;
				case 'curtainSliceRight':				arr = arr.reverse();				break;
				case 'blindCurtainTopLeft':			break;
				case 'blindCurtainBottomLeft':		arr = arr.reverse();				break;
				case 'blindCurtainSliceTop':			break;
				case 'blindCurtainTopRight':		break;
				case 'blindCurtainBottomRight':	arr = arr.reverse();				break;
				case 'blindCurtainSliceBottom':	arr = arr.reverse();				break;
				case 'stampede':							arr = shuffle(arr);				break;
				case 'mosaic':						break;
				case 'mosaicReverse':					arr = arr.reverse();				break;
				case 'mosaicRandom':					arr = shuffle(arr);				break;
				case 'mosaicSpiral':
			...
			$.each(arr, function(index, value) {
				...
				addLeft = (value%cols<leftScrap) ? 1 : 0 ;
				if(value%cols==0){
					tAppW = 0;
				}
				addTop = (Math.floor(value/cols)<topScrap) ? 1 : 0 ;
				...
				switch(fx){
					...
					case 'curtainSliceRight':
						height = 0,
						width = Math.floor((w/cols)+addLeft+1);
						marginTop = (value%2==0) ? Math.floor((h/rows)+addTop+1)+'px' : '-'+Math.floor((h/rows)+addTop+1)+'px' ;
							break;
					case 'blindCurtainTopRight':
					case 'blindCurtainBottomRight':
						height = Math.floor((h/rows)+addTop+1),
					...
					case 'blindCurtainSliceBottom':
						height = Math.floor((h/rows)+addTop+1),
						width = 0;
						marginLeft = (value%2==0) ? '-'+Math.floor((w/cols)+addLeft+1)+'px' : Math.floor((w/cols)+addLeft+1)+'px' ;
							break;
					case 'stampede':
						height = 0;
						width = 0;					
								...
								function(){										
									if (($.browser.msie && $.browser.version < 9) || (opts.loader == 'bar' )) {								
										if(rad<=1 && !elem.hasClass('stopped')){
											...
											if(!elem.hasClass('stopped')){	imgFake();}
											clearInterval(u);
