...
!function (a) {
	...
	b.prototype.initTable = function () {
		...
		this.options.editable && a.each(this.options.columns, function (a, c) {
			if (c.editable) {
				var d = c.formatter;
				c.formatter = function (a, e, f) {
					var g = d ? d(a, e, f) : a;
					return ['<a href="javascript:void(0)"', ' data-name="' + c.field + '"', ' data-pk="' + e[b.options.idField] + '"', ">" + g + "</a>"].join("")
				}
			}
		})
	},
	b.prototype.initBody = function () {
		...
		this.options.editable && a.each(this.options.columns, function (a, c) {
			c.editable && b.$body.find('a[data-name="' + c.field + '"]').editable(c.editable)
		})
	}
}
(jQuery);
