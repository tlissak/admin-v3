...
// $.fn.mColorPicker.init.elm_replace = '.myclass'
...
// $.fn.mColorPicker.init.elm_replace = false
...
(function($){
  ...
  $.fn.mColorPicker.liveEvents = function() {
    ...
    if ($.fn.mColorPicker.init.checkRedraw && $.fn.mColorPicker.init.elm_replace) {
      $(document).bind($.fn.mColorPicker.init.checkRedraw + '.mColorPicker', function () {		  
        $('input[data-mcolorpicker!="true"]').filter(function() { 				
          return ($.fn.mColorPicker.init.elm_replace == '[type=color]')? $(this).data('type') : $(this).is($.fn.mColorPicker.init.elm_replace);
        }).mColorPicker();
      });
    }
  };
  ...
  $.fn.mColorPicker.drawPickerTriggers = function ($t) {
    ...
    colorPicker = $('span#color_work_area').html() // .replace(/__type="color"/gi, '')
	.replace(/input /gi, (hidden)? 'input type="hidden"': 'input type="text"');
    ...
    if (hidden) {
      $('#icp_' + id).css({       'background-color': color,        'background-image': image,        'display': 'inline-block'   })
	  .attr(       'class', $('#' + id).attr('class')  )  .addClass(    'mColorPickerTrigger'   );
    } else {

      $('#' + id).css({        'background-color': color,        'background-image': image      })
	  .css({        'color': $.fn.mColorPicker.textColor($('#' + id).css('background-color'))
      }).after(
  ...
  $.fn.mColorPicker.drawPicker = function () {
    ...
    $('#mColorPicker').css({      'border':'1px solid #ccc',      'color':'#fff',      'z-index':999998,      'width':'194px',
      'height':'184px',      'font-size':'12px',      'font-family':'times'    });
    $('.mPastColor').css({      'height':'18px',      'width':'18px',      'border':'1px solid #000',      'float':'left'    });
    $('#colorPreview').css({      'height':'50px'    });
    $('.mNoLeftBorder').css({      'border-left':0    });
    $('.mClear').css({      'clear':'both'    });
    $('#mColorPickerWrapper').css({      'position':'relative',      'border':'solid 1px gray',      'z-index':999999    });    
    $('#mColorPickerImg').css({      'height':'128px',      'width':'192px',      'border':0,      'cursor':'crosshair',
      'background-image':"url('" + $o.imageFolder + "colorpicker.png')"    });    
    $('#mColorPickerImgGray').css({      'height':'8px',      'width':'192px',      'border':0,      'cursor':'crosshair', 
	  'background-image':"url('" + $o.imageFolder + "graybar.jpg')"    });    
    $('#mColorPickerInput').css({      'border':'solid 1px gray',      'font-size':'10pt',      'margin':'3px',      'width':'80px'    });    
    $('#mColorPickerImgGrid').css({      'border':0,      'height':'20px',      'width':'20px',      'vertical-align':'text-bottom'    });    
    $('#mColorPickerSwatches').css({      'border-right':'1px solid #000'    });    
    $('#mColorPickerFooter').css({      'background-image':"url('" + $o.imageFolder + "grid.gif')",      'position': 'relative',
      'height':'26px'    });
    ...
    var swatch = $.fn.mColorPicker.getCookie('swatches'),     i = 0;
  ...
  $.fn.mColorPicker.colorShow = function (id) {
	  ...
	  //bug fixed
	  //the outerHeight return $object	
    var $e = $("#icp_" + id);
        pos = $e.offset(),
        $i = $("#" + id);
        hex = $i.data('hex') || $i.attr('hex'),
        pickerTop = pos.top + $e.height(),
        pickerLeft = pos.left,
        $d = $(document),
        $m = $("#mColorPicker");
    ...
    if ($i.attr('disabled')) return false;
                ...
                if (pickerTop + $m.height() > $d.height()) pickerTop = pos.top - $m.height() + $e.height();
                if (pickerLeft + $m.width() > $d.width()) pickerLeft = pos.left - $m.width() + $e.width();
    $m.css({
      'top':(pickerTop) + "px",
      'left':(pickerLeft) + "px",
      'position':'absolute'
    ...
    $("#mColorPickerBg").css({      'z-index':999990,      'background':'black',      'opacity': .01,
      'position':'absolute',      'top':0,      'left':0,      'width': parseInt($d.width(), 10) + 'px',
    ...
    if ($('#' + id).data('text')) $.fn.mColorPicker.currentColor = $e.css('background-color');
  ...
  $.fn.mColorPicker.setInputColor = function (id, color) {  
	...
	var textColor = $.fn.mColorPicker.textColor(color); 
    if ($('#' + id).data('text') || $('#' + id).attr('text')) 
			$("#icp_" + id).css({'background-color': color, 'background-image': image});
	var hexColor  = $.fn.mColorPicker.RGBtoHex(color) ;
    $("#" + id).val(hexColor).css({'background-color': color, 'background-image': image, 'color' : textColor}).trigger('change');
  ...
  $.fn.mColorPicker.addToSwatch = function (color) {  
    ...
    $('.mPastColor').each(function() {  
      ...
      if ($.fn.mColorPicker.color != swatch[0] && $.fn.mColorPicker.RGBtoHex($.fn.mColorPicker.color) 
	  != swatch[0] && $.fn.mColorPicker.hexToRGB($.fn.mColorPicker.color) 
	  != swatch[0] && swatch.length < 10) 
  ...
  $.fn.mColorPicker.whichColor = function (x, y, hex) {
    ...
    if (x < 32) {     colorG = x * 8;      colorB = 0;
	} else if (x < 64) {        colorR = 256 - (x - 32 ) * 8;      colorB = 0;
	} else if (x < 96) {      colorR = 0;      colorB = (x - 64) * 8;
	} else if (x < 128) {      colorR = 0;      colorG = 256 - (x - 96) * 8;
    } else if (x < 160) {        colorR = (x - 128) * 8;      colorG = 0;
	} else {    colorG = 0;      colorB = 256 - (x - 160) * 8;    }
