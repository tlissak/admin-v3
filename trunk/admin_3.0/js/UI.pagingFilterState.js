(function(ui){	
	ui.docLoad(function(){	// Paging 
		...
		$(document).on('keyup',".form-filter input",function(){
            var that = this ;
            if (window.timeout_filter) clearTimeout(window.timeout_filter) ;
            window.timeout_filter = setTimeout(function(){
                ui.getFilterPagingList.call($(that).closest('.context'), 0);
            },300);
